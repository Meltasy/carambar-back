# carambar-back

[![License ISC](https://img.shields.io/github/license/Meltasy/blog-api)](https://opensource.org/license/isc-license-txt)
[![ECMAScript](https://img.shields.io/badge/ECMAScript-2025-blue.svg)](https://ecma-international.org/publications-and-standards/standards/ecma-262/)
[![Node.js](https://img.shields.io/badge/Node.js-v22.12.0-brightgreen.svg)](https://nodejs.org/)
[![npm](https://img.shields.io/badge/npm-v11.3.0-red.svg)](https://www.npmjs.com/)
[![Repo Size](https://img.shields.io/github/repo-size/Meltasy/blog-api)](https://github.com/Meltasy/carambar-back)

API backend pour une mini application web de blagues.

Découvrez [Carambar Blagues](https://carambar-back-5c9h.onrender.com/v1/) !

Lire la [documentation API](https://carambar-back-5c9h.onrender.com/api-docs/).

## Tech Stack

* Node.js avec le framework Express.js
* Sequelize ORM avec base de données SQLite

## License

L'ensemble de ce projet est sous licence ISC. Pour plus d'informations, consultez le fichier [LICENSE](LICENSE).
